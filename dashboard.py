...
PAGE = PAGE.replace("</script>", r"""
function taskCard(task){const card=make('details','task-card'),summary=make('summary','task-head');summary.append(make('b','',task.title),make('span','chip '+task.status,task.status));card.append(summary);const info=make('div','task-info');[['Status',task.status],['Weight',task.weight],['Due',task.due||'No due date'],['Assignment',task.assigned?'Assigned':'Unassigned']].forEach(([label,value])=>{const row=make('span','');row.append(make('strong','',label+': '),document.createTextNode(value));info.append(row)});card.append(info);return card}
function projectCard(project){const card=make('details','project project-card'),summary=make('summary','project-head');summary.append(make('h3','',project.name),make('span','pill',project.progress.pct+'%'));card.append(summary);const body=make('div','project-body');body.append(make('p','',project.description||'No description yet.'));body.append(difficulty(project.difficulty));body.append(bar(project.progress.pct));const statuses=make('div','task-summary');[['doing',project.progress.doing],['blocked',project.progress.blocked],['todo',project.progress.todo],['done',project.progress.done]].forEach(([status,count])=>{if(count)statuses.append(make('span','chip '+status,`${count} ${status}`))});body.append(statuses);const tasks=make('div','task-list');tasks.append(make('div','label',project.tasks.length?'Tasks — select one for details':'No tasks yet.'));project.tasks.forEach(task=>tasks.append(taskCard(task)));body.append(tasks);card.append(body);return card}
function projects(items,query=''){const wrap=el('projects');wrap.replaceChildren();const shown=items.filter(p=>(p.name+' '+p.description+' '+p.tasks.map(t=>t.title).join(' ')).toLowerCase().includes(query.toLowerCase()));if(!shown.length){wrap.append(make('div','empty','No projects match that filter.'));return}shown.forEach(project=>wrap.append(projectCard(project)))}
function showMilestone(data,node,source){let detail=el('tree-detail');if(detail&&detail.dataset.key===node.key){detail.remove();source.setAttribute('aria-expanded','false');return}if(!detail){detail=make('aside','tree-detail');detail.id='tree-detail';el('tree-section').append(detail)}document.querySelectorAll('.node[aria-expanded="true"]').forEach(card=>card.setAttribute('aria-expanded','false'));source.setAttribute('aria-expanded','true');detail.dataset.key=node.key;detail.replaceChildren();const top=make('div','detail-top');top.append(make('div','eyebrow','Milestone details'),make('button','detail-close','Close'));top.lastChild.type='button';top.lastChild.onclick=()=>{source.setAttribute('aria-expanded','false');detail.remove()};detail.append(top,make('h3','',node.name),make('p','',node.description||'No description yet.'),make('p','label','What this opens: '+(node.unlocks||'No stated unlock.')));const linked=data.projects.filter(project=>node.projects.includes(project.name));if(!linked.length){detail.append(make('div','empty','No public project is linked to this milestone.'));return}detail.append(make('h4','',linked.length===1?'Linked project':'Linked projects'));linked.forEach(project=>detail.append(projectCard(project)));detail.scrollIntoView({behavior:'smooth',block:'nearest'})}
function refresh(data){const view=filtered(data),tree=el('tree'),saved=tree.value;tree.replaceChildren(new Option('All milestones',''));view.trees.forEach(t=>tree.append(new Option(t.name,t.key)));tree.value=[...tree.options].some(option=>option.value===saved)?saved:'';const current=view.trees.find(t=>t.key===tree.value),shown=view.milestones.filter(node=>!tree.value||current.members.includes(node.key));draw(view,tree.value);el('tree-detail')?.remove();[...el('graph').querySelectorAll('.node')].forEach((card,index)=>{const node=shown[index];card.classList.add('clickable');card.tabIndex=0;card.setAttribute('role','button');card.setAttribute('aria-expanded','false');const open=()=>showMilestone(view,node,card);card.onclick=open;card.onkeydown=event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();open()}}});overview(view);projects(view.projects,el('search').value)}
</script>""")
